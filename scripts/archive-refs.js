 ...
 *
 * Zero dependencies. Reads the `references[]` array of data/forum.json,
 ...
 *
 * This is the shared template copy (cronologia/core template/scripts/), with
 * this repo's differences declared in ADOPT blocks — see core#33. The country
 * dossiers' bare-URL sources are covered by the repo-owned companion,
 * scripts/archive-country-sources.js, which writes into the same cache.
 *
...
const USER_AGENT =
  'cronologia-archive-refs/1.0 (Cronologia Foro de Sao Paulo chronology; +https://github.com/cronologia/fsp)';
// <<< ADOPT
...
async function main() {
  ...
  for (const ref of references) {
    ...
    // >>> ADOPT: official-ref
    // Which references get a fresh re-capture. fsp marks them `official: true`,
    // EXCEPT on forodesaopaulo.org: that site is geoblocked to non-Brazilian
    // IPs, so a Save Page Now re-crawl (foreign egress) would replace a good
    // snapshot with the blocked page. Those keep their citation-time capture.
    const host = (() => { try { return new URL(url).hostname; } catch { return ''; } })();
    const isOfficial = ref.official === true && !/(^|\.)forodesaopaulo\.org$/i.test(host);
    // <<< ADOPT
